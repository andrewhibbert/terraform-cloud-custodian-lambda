...
func TestPeriodicExample(t *testing.T) {
	...
	var ownerIDFilter struct {
		Type  string `json:"type"`
		Key   string `json:"key"`
		Op    string `json:"op"`
		Value string `json:"value"`
	}
	require.NoError(t, json.Unmarshal([]byte(rawFilter), &ownerIDFilter))

	prettyFilter, err := json.MarshalIndent(ownerIDFilter, "", "  ")
	require.NoError(t, err)
	t.Logf("resolved OwnerId filter in config.json:\n%s", prettyFilter)

	assert.Regexp(t, `^\d+$`, ownerIDFilter.Value, "the resolved account_id should be numeric")
