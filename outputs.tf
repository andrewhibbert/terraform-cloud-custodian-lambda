...
output "periodic_event_rule" {
  ...
  value       = local.periodic_mode ? aws_cloudwatch_event_rule.eventbridge : {}
}

output "periodic_event_rule_name" {
  description = "The name of the CloudWatch Event Rule for periodic mode"
  value       = local.periodic_mode ? { for region, rule in aws_cloudwatch_event_rule.eventbridge : region => rule.name } : {}
}

output "periodic_event_rule_arn" {
  description = "The ARN of the CloudWatch Event Rule for periodic mode"
  value       = local.periodic_mode ? { for region, rule in aws_cloudwatch_event_rule.eventbridge : region => rule.arn } : {}
}

output "periodic_schedule_expression" {
  description = "The schedule expression for periodic mode"
  value       = local.periodic_mode ? { for region, rule in aws_cloudwatch_event_rule.eventbridge : region => rule.schedule_expression } : {}
}
...
output "cloudwatch_event_rule" {
  ...
  value       = local.cloudwatch_event_mode ? aws_cloudwatch_event_rule.eventbridge : {}
}

output "cloudwatch_event_rule_name" {
  description = "The name of the CloudWatch Event Rule for event mode"
  value       = local.cloudwatch_event_mode ? { for region, rule in aws_cloudwatch_event_rule.eventbridge : region => rule.name } : {}
}

output "cloudwatch_event_rule_arn" {
  description = "The ARN of the CloudWatch Event Rule for event mode"
  value       = local.cloudwatch_event_mode ? { for region, rule in aws_cloudwatch_event_rule.eventbridge : region => rule.arn } : {}
}

output "cloudwatch_event_pattern" {
  description = "The event pattern for event mode"
  value       = local.cloudwatch_event_mode ? { for region, rule in aws_cloudwatch_event_rule.eventbridge : region => rule.event_pattern } : {}
}
